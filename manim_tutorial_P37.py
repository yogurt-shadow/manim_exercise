...
class Hanger_Shapes(Scene):
    def construct(self):
        a = []
        for i in range(-6, 5):
            triangle=Polygon(np.array([i, 0, 0]), np.array([i + 1, 1, 0]), np.array([i + 1, -1, 0]))
            a.append(triangle)

        self.play(ShowCreation(a[0]))
        self.add(Polygon(np.array([-6, 0, 0]), np.array([-5, 1, 0]), np.array([-5, -1, 0])))
        for i in range(len(a) - 1):
            square = Polygon(np.array([i - 6, -1, 0]), np.array([i - 6, 1, 0]), np.array([i - 5, 1, 0]), np.array([i - 5, -1, 0]))
            self.play(Transform(a[i], square))
            self.play(Transform(square,a[i + 1]))

        j = len(a) - 7

        self.play(Transform(a[-1], Polygon(np.array([j, -1, 0]), np.array([j, 1, 0]), np.array([j + 1, 1, 0]), np.array([j + 1, -1, 0]))))

class Hanger_Circles(Scene):
    def construct(self):
        circle = Circle(radius = 1, color=PURPLE_A)
        self.play(ShowCreation(circle))
        for i in range(1, 7):
             circle = Circle(radius = i, color=PURPLE_A)
             circle2 = Circle(radius = i + 1, color = PURPLE_A)
